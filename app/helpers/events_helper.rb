	...
	def self.scrape_choose_chicago 
		...
		open(choose_chicago_url) do |rss|
		  ...
		  feed.items.each do |event|
		  	...
		  	Capybara.app_host = URI.parse(url).host

		  	visit(url)
		  	img_url = find(:xpath, '//div[@class="mapContainer"]//img')['src']

		  	result_hash = {date: date, title: title, desc: desc, url: url, img_url: img_url}

		  	Event.create(result_hash) if !Event.exists?(title: title, desc: desc, url: url, img_url: img_url)
